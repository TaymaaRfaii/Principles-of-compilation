...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmIf (test, if_then, if_else) -> ScmIf' (run test params env, run if_then params env, run if_else params env) 
      | ScmSeq exp_list -> ScmSeq' (List.map (fun exp -> run exp params env) exp_list) 
      | ScmOr exp_list -> ScmOr' (List.map (fun exp -> run exp params env) exp_list)  
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      | ScmIf' (test, if_test, if_not_test) ->
        ScmIf' (run false test, run in_tail if_test, run in_tail if_not_test)
      | ScmSeq' exp_list -> ScmSeq' (runl in_tail (List.hd exp_list) (List.tl exp_list))
      | ScmOr' exp_list -> ScmOr' (runl in_tail (List.hd exp_list) (List.tl exp_list))
      ...
      | ScmLambda' (params, lambda_kind, body) ->
        let body' = run true body in
        (match lambda_kind with
         | Simple -> ScmLambda' (params, Simple, body')
         | Opt optionals -> ScmLambda' (params, Opt optionals, body'))
...
let sprint_exprs' chan exprs =
 ...
 (* here the add-to-compiler starts *)
       ...
       module Code_Generation (* : CODE_GENERATION *) = struct
         ...
         let const_repr sexpr loc table = match sexpr with
           ...
           | ScmString str ->
             (* our implemntation *)
              let c = String.length str in
              ([RTTI "T_string"; Quad c; ASCII str], c + word_size + 1)
           | ScmSymbol sym ->
           ...
           | ScmVector s ->
             (* our implemntation *)
             let addrs =
               List.map (fun se -> ConstPtr (search_constant_address se table)) s in
               let count = List.length s in
               ((RTTI "T_vector") :: (Quad count) :: addrs,
                1 + (count + 1) * word_size)           
            | ScmPair (car, cdr) ->
             (* our implemntation *)
             let car_addr = search_constant_address car table in
             let cdr_addr = search_constant_address cdr table in
             ([RTTI "T_pair"; ConstPtr car_addr; ConstPtr cdr_addr], 1 + 2 * word_size)
        ...
        let make_label_exact_arguments_loop =
            make_make_label ".L_lambda_opt_exact_arguments_loop";;      
        let make_label_exact_arguments_loop_exit =
          make_make_label ".L_lambda_opt_exact_arguments_loop_exit";;
        let make_label_creat_list_loop =
            make_make_label ".L_lambda_opt_creat_list_loop";;
        let make_label_creat_list_loop_exit =
            make_make_label ".L_lambda_opt_creat_list_loop_exit";;        
        let make_label_more_arguments_loop =
              make_make_label ".L_lambda_opt_more_arguments_loop";;
        let make_label_more_arguments_loop_exit =
              make_make_label ".L_lambda_opt_more_arguments_loop_exit";;
           ...
           let rec run params env = function
             ...
             | ScmOr' exprs' ->
                   ...
                   let asm_code =
                     ...
                     | None -> run params env (ScmConst' (ScmBoolean false)) in
                   asm_code
             ...
             | ScmBoxSet' (var', expr') ->
                 ...
                 let new_value = run params env expr' in
                 new_value ^
                 "\tpush rax\n" ^
                 box ^
                 "\tpop qword [rax]\n" ^
            ...
            | ScmLambda' (params', Opt opt, body) ->
                  ...
                  and label_end = make_lambda_opt_end () in
                    ...
                    label_opt_code (List.length params') label_arity_exact label_arity_more) ^
                    (Printf.sprintf "%s:\n
                    mov rdx, 3 + %d\n
                    sub rsp, 8\n
                    mov qword rbx, rsp\n"
                    label_arity_exact (List.length params')) ^
                    (Printf.sprintf "%s:\n
                    mov qword rcx, [rbx + 8]\n
                    mov qword [rbx], rcx\n
                    add rbx, 8\n
                    ...
                    cmp rdx, 0\n
                    jne %s\n"
                    label_loop_opt_param_loop label_loop_opt_param_loop) ^
                    (Printf.sprintf "%s:\n
                    inc r15\n
                    ...
                    jmp %s\n"
                    label_loop_opt_params_end label_stack_ok) ^
                    (Printf.sprintf "%s:\n
                    mov r13, [rsp + 2 * 8]\n
                    mov rax, sob_nil\n
                    mov r15, [rsp + 2 * 8]\n
                    lea rbx, [rsp + 8 * (2 + r15)]\n
                    sub r15, %d\n"
                    label_arity_more (List.length params')) ^
                    (Printf.sprintf "%s:\n
                    mov rcx, rax\n
                    mov rdx, [rbx]\n
                    mov rdi, (8 * 2 + 1)\n
                    ...
                    cmp r9, 0\n
                    jne %s\n"
                    label_loop_params2 label_loop_params2) ^
                    (Printf.sprintf "%s:\n
                    add rsp, rbx\n"
